 ...
 * user.
 */
public class UserCreateOrEditAndLoginTask extends
		AsyncTask<Void, Void, Boolean> {
	private CreateOrEditUserActivity activity;
	private Boolean editMode;
	private User creatingUser;

	public UserCreateOrEditAndLoginTask(CreateOrEditUserActivity activity,
			Boolean editMode, User creatingUser) {
	...
	protected Boolean doInBackground(Void... params) {
		...
		try {
			...
			if (editMode) {
				...
				creatingUser = myapp.getUserController().createUser(
						creatingUser);
			}
	...
	protected void onPostExecute(final Boolean success) {
		...
		if (success) {
			...
			if (!editMode) {
				myIntent = new Intent(activity.getApplicationContext(),
						MainActivity.class);
				...
				Toast.makeText(activity.getApplicationContext(),
						"Votre compte a été modifié !", 5000);
			}
			...
			alertDialogBuilder
					.setTitle("Erreur de communication avec le serveur")
					.setMessage("Veuillez rééssayer.").setCancelable(false)
					.create().show();
