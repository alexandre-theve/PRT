...
public class AssyncLogUser extends AsyncTask<Void, Void, Boolean> {
	...
	protected Boolean doInBackground(Void... params) {
		...
		SharedPreferences prefs = andrEvents.getSharedPreferences(
				activity.getString(R.string.app_package), Context.MODE_PRIVATE);
		if (andrEvents.getUserController().getUserConnected() != null) {
			...
			logingUser = andrEvents.getUserController().getById(
					prefs.getInt("loggedUserId", -1));
	...
	protected void onPostExecute(Boolean result) {
		...
		if (logingUser != null) {
			MyApplication andrEvents = (MyApplication) activity
					.getApplication();
